...
mongoose.connect('mongodb+srv://rahul199202012:gexBdbMGUqtwE3Nq@cluster0.k7xol6w.mongodb.net/birthcertificate', {
...
app.get('/birthcertificate', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'birthcertificate.html'));
});
...
app.post("/birthcertificate", checkAuth, async (req, res) => {
  const { fullName, aadharNumber, dob, gender, fatherName, fatherAadhar, motherName, motherAadhar, purpose, address, email, pinCode } = req.body;
  ...
  try {
      ...
      const birthCertificate = new BirthCertificate({
          userId: user._id, fullName, aadharNumber, dob, gender, fatherName, fatherAadhar, motherName, motherAadhar, purpose, address, email, pinCode
      });
      await birthCertificate.save();

      const transaction = new Transaction({
          userId: user._id, amount: requiredBalance, type: "debit", description: "Birth Certificate Application Fee"
      });
      await transaction.save();

      res.status(201).json({ message: "Birth Certificate Application Submitted Successfully!" });
  } catch (error) {
...
app.get('/api/birthcertificate', async (req, res) => {
  try {
    const birthCertificates = await BirthCertificate.find(); // MongoDB से डेटा लाएं
    res.json(birthCertificates);
  } catch (error) {
    console.error('Database fetch error:', error);
    res.status(500).json({ message: 'Server error fetching birth certificates' });
  }
});
...
app.get('/api/data/:collectionName', async (req, res) => {
  ...
  try {
      ...
      // Agar collection 'birthcertificate' hai, toh data waise ka waisa bhejna hai
      if (collectionName === 'birthcertificate') {
          return res.json(data);
      }

      // Baaki collections ke liye agar data na mile toh error response bhejna hai
      if (!data.length) {
